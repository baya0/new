...
export default function HomePage() {
  return (
    <>
      {/* ════════ HERO — Immersive with floating blobs ════════ */}
      <section className="relative overflow-hidden hero-bg flex items-center" style={{ minHeight: "calc(100vh - 72px)" }}>
        ...
        <div className="relative max-w-6xl mx-auto px-6 lg:px-8 py-24 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center w-full">
          <div>
      ...
      {/* ════════ WHO WE ARE — overlapping glass panel ════════ */}
      <section className="relative section-depth" style={{ paddingTop: 140, paddingBottom: 140, paddingLeft: 24, paddingRight: 24 }}>
        <div className="blob blob-blue w-[500px] h-[500px]" style={{ left: -200, top: 100 }} />

        <div className="max-w-6xl mx-auto">
          <div className="float-panel p-10 lg:p-14">
      ...
      {/* ════════ SERVICE CARDS — Glass bento ════════ */}
      <section className="relative section-depth" style={{ padding: "100px 24px 140px" }}>
        <div className="blob blob-cyan w-[450px] h-[450px]" style={{ right: -150, top: 50 }} />
        <div className="blob blob-purple w-[300px] h-[300px]" style={{ left: "20%", bottom: -80 }} />
            ...
            {th.services.map((svc, i) => {
              ...
              return (
                <StaggerChild key={i} i={i}>
                  <div className="glass-card p-9 relative overflow-hidden group h-full">
                    ...
                    <div className="absolute -top-20 -right-20 w-48 h-48 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" style={{ background: `radial-gradient(circle, ${colorMap[svc.color]}18, transparent 70%)` }} />

                    <div className="w-[72px] h-[72px] rounded-2xl flex items-center justify-center mb-7 transition-all duration-400 group-hover:scale-110 group-hover:shadow-lg" style={{ background: `${colorMap[svc.color]}12`, border: `1px solid ${colorMap[svc.color]}25`, boxShadow: `0 4px 16px ${colorMap[svc.color]}10` }}>
                      <Icon size={30} style={{ color: colorMap[svc.color] }} />
                    </div>
                    <h3 className="text-xl font-bold mb-3 leading-snug" style={{ color: "var(--white)" }}>{svc.title}</h3>
                    <p className="text-sm leading-[1.8] mb-6" style={{ color: "var(--w55)" }}>{svc.desc}</p>
                    <div className="tag" style={{ background: `${colorMap[svc.color]}10`, border: `1px solid ${colorMap[svc.color]}22`, color: colorMap[svc.color] }}>{svc.tag}</div>
                  </div>
                </StaggerChild>
              );
            })}
      ...
      {/* ════════ OUR SERVICES — Floating panel ════════ */}
      <section className="relative section-depth" style={{ padding: "100px 24px 140px" }}>
        <div className="blob blob-blue w-[400px] h-[400px]" style={{ right: -120, bottom: 0 }} />
        ...
        <div className="relative overflow-hidden">
          <div className="marquee-track">
            {[...th.clients, ...th.clients, ...th.clients, ...th.clients].map((c, i) => (
              <span key={i} className="text-3xl lg:text-4xl font-black opacity-20 hover:opacity-60 transition-all duration-500 cursor-default shrink-0" style={{ color: "var(--w85)" }}>{c}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="relative section-depth" style={{ padding: "120px 24px 140px" }}>
        <div className="blob blob-purple w-[400px] h-[400px]" style={{ right: -80, top: 100 }} />
        <div className="blob blob-blue w-[300px] h-[300px]" style={{ left: -100, bottom: 50 }} />
              ...
              <div className="tl-timeline">
                {th.timeline.map((item, i) => (
                  <div key={i} className="tl-item relative mb-6">
                    ...
                    <div className="glass-card rounded-2xl p-6 !transform-none !rounded-2xl" style={{ borderLeft: `4px solid ${colorMap[item.color]}` }}>
                      <div className="text-[15px] font-bold" style={{ color: "var(--white)" }}>{item.title}</div>
                      <div className="text-sm mt-2 leading-relaxed" style={{ color: "var(--w55)" }}>{item.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* ════════ TESTIMONIAL — Floating glass ════════ */}
      <section className="relative" style={{ padding: "100px 24px 120px" }}>
        <div className="blob blob-cyan w-[350px] h-[350px]" style={{ left: "50%", top: 0, transform: "translateX(-50%)" }} />

        <AnimatedSection className="max-w-3xl mx-auto relative">
          <div className="float-panel p-10 lg:p-14 relative overflow-hidden">
      ...
      {/* ════════ CTA — Dramatic closing ════════ */}
      <section className="relative overflow-hidden hero-bg" style={{ padding: "140px 24px" }}>
