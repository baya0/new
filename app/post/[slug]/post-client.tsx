...
function buildComponents(cc: string): PortableTextComponents {
  return {
    block: {
      normal: ({ children }) => (
        <p className="text-[15px] leading-[1.9] mb-6" style={{ color: "var(--w55)" }}>{children}</p>
      ),
      h2: ({ children }) => (
        <h2 className="text-lg font-bold mb-4 mt-10" style={{ color: "var(--white)" }}>{children}</h2>
      ),
      h3: ({ children }) => (
        <h3 className="text-base font-bold mb-3 mt-8" style={{ color: "var(--white)" }}>{children}</h3>
      ),
      blockquote: ({ children }) => (
        <blockquote
          className="border-l-2 pl-5 italic my-6 text-[15px] leading-relaxed"
          style={{ borderColor: cc, color: "var(--w55)" }}
        >
          {children}
        </blockquote>
      ),
    ...
    list: {
      bullet: ({ children }) => (
        <ul className="space-y-3 mb-6 ml-1">{children}</ul>
      ),
      number: ({ children }) => (
    ...
    listItem: {
      bullet: ({ children }) => (
        <li className="flex gap-3 items-start text-[15px] leading-relaxed" style={{ color: "var(--w55)" }}>
      ...
      number: ({ children, index }) => (
        <li className="flex gap-3 items-start text-[15px] leading-relaxed" style={{ color: "var(--w55)" }}>
          <span
            ...
            style={{ background: `${cc}10`, color: cc, border: `1px solid ${cc}20` }}
          >
            {(index ?? 0) + 1}
          </span>
          <span className="flex-1">{children}</span>
    ...
    marks: {
      ...
      link: ({ children, value }) => (
        <a
          ...
          style={{ color: cc, textDecoration: "underline" }}
        >
          {children}
        </a>
      ),
...
export default function PostClient({
  ...
  const components = buildComponents(cc);
