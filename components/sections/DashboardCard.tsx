...
export default function DashboardCard({ t }: DashboardCardProps) {
  ...
  return (
    <div
      className="float-panel rounded-2xl p-7 animate-float relative overflow-hidden"
    >
      {/* Top accent bar */}
      ...
      <div className="relative grid grid-cols-2 gap-3 mb-6">
        {dash.metrics.map((m: any, i: number) => (
          <div key={i} className="rounded-xl p-4 transition-all duration-300 hover:scale-[1.02]"
      ...
      <div className="relative flex items-end gap-[3px] h-[72px] mb-5 p-2 rounded-xl" style={{ background: "var(--bg3)", border: "1px solid var(--border)" }}>
        {BAR_HEIGHTS.map((h, i) => (
          <div key={i} className="flex-1 rounded-sm transition-all duration-500 hover:brightness-110"
            style={{
              height: `${h}%`,
        ...
        {dash.statuses.map((s: any, i: number) => (
          <div key={i} className="flex justify-between items-center py-2.5 text-[11px]" style={{ borderTop: "1px solid var(--border)" }}>
            <span className="flex items-center gap-2.5 font-medium" style={{ color: "var(--w55)" }}>
              <span className="w-2 h-2 rounded-full inline-block" style={{ background: colorMap[s.color] }} />
              {s.label}
            </span>
