...
export default function DashboardCard({ t }: DashboardCardProps) {
  ...
  return (
    <div
      className="float-panel rounded-3xl p-7 animate-float relative overflow-hidden"
    >
      {/* Top gradient bar */}
      <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: "var(--gradient-brand)" }} />

      {/* Ambient glow */}
      <div className="absolute -top-20 -right-20 w-40 h-40 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(29,107,196,0.1), transparent 70%)" }} />

      {/* Header */}
      <div className="relative flex justify-between items-center mb-6 pb-4" style={{ borderBottom: "1px solid var(--glass-card-border)" }}>
      ...
      <div className="relative flex items-end gap-[3px] h-[72px] mb-5 p-2 rounded-2xl" style={{ background: "var(--glass-card)", backdropFilter: "blur(8px)", border: "1px solid var(--glass-card-border)" }}>
        {BAR_HEIGHTS.map((h, i) => (
      ...
      <div className="relative">
        {dash.statuses.map((s: any, i: number) => (
          <div key={i} className="flex justify-between items-center py-2.5 text-[11px]" style={{ borderTop: `1px solid var(--glass-card-border)` }}>
